...
# Filter for only isolate CTRL --------------------------------------------
FCTRL.samples <- get_samples(metadata, Treatment = 'Fungi', value = 'CTRL')

norm.matrix<-norm.matrix%>%
  select(FCTRL.samples)
...
fileName = paste(figures_dir, 'CTRL_CCA_Plot.png',sep = '/')
...
plot2
#Save to figures folder
fileName = paste(figures_dir, 'CTRL_CCA_FeatureArrows.png',sep = '/')
