...
plot1<-ggplot()+
  ...
  geom_segment(data = centroids, aes(x = 0, y = 0, xend = (CCA1),
                                     yend = (CCA2)), arrow = arrow(length = unit(1/2, "picas")),
               color = "black")+
  ...
  theme_classic()+
  labs(title= "Isolate 1154", caption= "Plant P = 0.016")
plot1
#Save to figures folder
fileName = paste(figures_dir, '1154_CCA_Plot.png',sep = '/')
...
plot2<- ggplot()+
  geom_point(data=sites, aes(CCA1, CCA2, color=Fungi, shape=Plant), size=3)+
  # scale_color_manual(values=colors)+
  geom_point(data=centroids, aes(CCA1, CCA2, color= 'green', size=2))+
 ...
 # geom_text_repel(data=features, 
  #                x = (features$CCA1*10), 
  #                y = (features$CCA2*10),
   #               label=features$Name, max.overlaps = 5, size=3)+
  
  theme_classic()
plot2
#Save to figures folder
fileName = paste(figures_dir, '1154_CCA_FeatureArrows.png',sep = '/')
