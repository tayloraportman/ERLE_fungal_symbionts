...
---
---
title: "CompoundDiscover data clean+combine"
output: html_notebook
---
...
# Import data tables
cd_results_file <- file.path(project_dir, 'data', 'RP_Compounds_KEGG_rem.1177_2.csv')
...
cd_results_table <- cd_results_table %>% filter(DeltaMass< .5,DeltaMass> -.5)
...
F1177_list <- compounds_table %>% 
  filter(Fungi == '1177') %>% 
  select(FeatureID) %>% 
  distinct()

F1246_list <- compounds_table %>% 
  filter(Fungi == '1246') %>% 
  select(FeatureID) %>% 
  distinct()

F1154_list <- compounds_table %>% 
  filter(Fungi == '1154') %>% 
  select(FeatureID) %>% 
  distinct()
