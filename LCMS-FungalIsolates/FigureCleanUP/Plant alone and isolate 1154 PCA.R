#add plant alone to isolate 1154
#Taylor Portman
# 3 March 2022
...
project_name <- '1154andCTRL_RP_LCMS_Germ'
figures_dir <- file.path(project_dir, paste0(project_name, '_output_figures'))
tables_dir <- file.path(project_dir,  paste0(project_name, '_output_tables'))
...
compounds_table<- subset(compounds_table, (Fungi=="1154"|Fungi=="CTRL"))
...
pca_plot <- plot_dotplot(pca_coordinates, PC1, PC2, Plant, Fungi) +
  labs(title = 'PCA plot',
       x = pc1,
       y = pc2)
pca_plot
