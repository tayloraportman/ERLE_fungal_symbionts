...
compounds_table <- read_csv("~/Google Drive/My Drive/projects/ERLE_fungal_symbionts/IsolatesAll_1154/RP_All_1154_output_tables/PhenylpropanoidBiosynthesis.csv")
...
phenyl.biosynthesis<- compounds_table%>%
  separate_rows(KEGGPathways, sep= ";")%>%
  filter(KEGGPathways=="Phenylpropanoid biosynthesis")

table_file <- file.path(tables_dir, 'PhenylpropanoidBiosynthesis.csv')
write_csv( phenyl.biosynthesis, table_file)
  
#```

```{r set_path, message=FALSE, warning=FALSE}
library(dplyr)
#compounds_table<-phenyl.biosynthesis
...
figure_file <- file.path(figures_dir, 'PhenylpropanoidBiosynthesis_heatmap.pdf')
...
```
```{r}
figure_file <- file.path(figures_dir, 'All_identified_features_heatmap_phenylpropBiosynthesis_.pdf')
