...
#set path variables
project_dir <- setwd("/Volumes/GoogleDrive/My Drive/projects/ERLE_fungal_symbionts/IsolatesAll_1154")

# Give a name for your project so the results are all grouped together in a directory with the same name
project_name <- 'RP_All_1154_checked'
...
cd_results_table <- cd_results_table %>% filter(Checked=="TRUE")
...
CTRL_list <- compounds_table %>% 
  filter(Plant == 'CTRL') %>% 
  select(FeatureID) %>% 
  distinct()

ERLE_list <- compounds_table %>% 
  filter(Plant == 'ERLE') %>% 
  select(FeatureID) %>% 
  distinct()

ERIN_list <- compounds_table %>% 
  filter(Plant == 'ERIN') %>% 
  select(FeatureID) %>% 
  distinct()

BOCU_list <- compounds_table %>% 
  filter(Plant == 'BOCU') %>% 
  select(FeatureID) %>% 
  distinct()

LEDU_list <- compounds_table %>% 
  filter(Plant == 'LEDU') %>% 
  select(FeatureID) %>% 
  distinct()

my_list <- list(CTRL = CTRL_list$FeatureID,
                ERLE = ERLE_list$FeatureID,
                ERIN = ERIN_list$FeatureID,
                BOCU = BOCU_list$FeatureID,
                LEDU = LEDU_list$FeatureID)

my_colors <- c('grey', 'pink', 'brown', 'blue', 'green')
...
distinct<- distinct(compounds_table, Name, .keep_all=TRUE)
#unique<- unique(cd_results_table$Name)

##284 Unique compounds
```